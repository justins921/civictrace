...
def member_money(c, cand_id, cycle):
    ...
    return c.execute(f"""
      SELECT k.filer_cmte_id, co.cmte_name, co.connected_org, co.org_tp, co.cmte_dsgn,
             ps.sector, ps.interest_side, ps.rule_id,
...
def build_trail(vote_key, bioguide, cand_id, cycle):
    ...
    # Split the sector's money by which side of the issue the giver sits on.
    OPPOSING = {"clean energy / environment"}
    supporting = [r for r in in_sector if (r["interest_side"] or "") not in OPPOSING]
    opposing = [r for r in in_sector if (r["interest_side"] or "") in OPPOSING]
    ...
    return {
        ...
        "money": {
            ...
            "aligned_side_dollars": round(sum(r["amount"] for r in supporting), 2),
            "opposed_side_dollars": round(sum(r["amount"] for r in opposing), 2),
            "pac_count": len(pacs),
            "pacs": pacs,
...
def alignment_label(t):
    """Deliberately conservative. Requires BOTH meaningful money and an
    unusual vote — a party-line vote tells you almost nothing about money."""
    ...
    if not unusual:
        return ("Party-line vote — low signal",
                f"{party_share}% of the member's own party voted the same way. A vote this "
                f"predictable is explained by party position, not by contributions.")
    if share >= 10 and m["aligned_side_dollars"] > m["opposed_side_dollars"]:
        return ("Notable overlap",
                f"The bill's sector supplied {share}% of this member's PAC money, and the vote "
                f"split the member's party ({party_share}% voted the same way). Worth reading "
                f"the underlying filings.")
    return ("Some overlap",
            f"Sector money is {share}% of this member's PAC total. Review the filings below.")
