...
def dump(name, table, rows, cols, per=400):
    for i in range(0, len(rows), per):
        vals = ",\n".join("(" + ",".join(q(r.get(k)) for k in cols) + ")" for r in rows[i:i + per])
        p = OUT / f"{name}_{i//per:03d}.sql"
        p.write_text(f'INSERT INTO civictrace.{table} ({",".join(cols)}) VALUES\n{vals}\n'
                     f'ON CONFLICT DO NOTHING;')
        FILES.append(p)
    print(f"{table}: {len(rows)} rows -> {math.ceil(len(rows)/per)} files")
