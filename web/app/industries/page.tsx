...
export default async function Industries() {
  const [{ data: sectors }, { data: cmtes }, { data: bills }] = await Promise.all([
    db.from('sector_profile').select('*').eq('cycle', CYCLE)
      .order('total_to_wi', { ascending: false }).order('sector', { ascending: true }),
    db.from('committee_profile').select('sector,interest_side,total_to_wi')
      .eq('cycle', CYCLE).gt('payments_to_wi', 0),
    db.from('bill_sector').select('sector,bill_key'),
  ])
